...
const values = [
  { title: "Integritas", desc: "Transparansi & kejujuran dalam setiap kemitraan bisnis" },
  { title: "Profesional", desc: "Tim berpengalaman dengan standar layanan tinggi" },
  { title: "Inovatif", desc: "Solusi teknologi terkini sesuai kebutuhan industri" },
  { title: "Solutif", desc: "Pendekatan problem-solving yang efektif & efisien" },
];
...
const contactStrip = [
  {
    ...
    label: "Alamat",
    value: "The Address @Cibubur Cluster Ultima B16, Jl. Leuwinanggung, Tapos — Depok",
  },
  {
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <section id="about" className="bg-[#F8F9FC] py-24 px-[5%]" ref={ref}>
      <div className="max-w-6xl mx-auto flex flex-col gap-12">
        ...
        <div className="max-w-3xl">
          ...
          >
            Mitra Bisnis Anda di<br />Ekosistem Pembayaran
          </h2>
          <p className="fade-up delay-2 text-[#4A5568] font-light leading-relaxed text-base text-justify max-w-2xl">
            CV Bangda Bertiga Karya adalah perusahaan solusi teknologi dan pembayaran yang berkomitmen
            menjadi penghubung antara kebutuhan bisnis Anda dengan teknologi terbaik. Kami hadir sebagai
        ...
        <div className="fade-up delay-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {values.map((v) => (
            ...
            >
              <div className="font-display font-bold text-[#081A3A] text-sm mb-2">{v.title}</div>
              <div className="text-[#4A5568] text-xs leading-relaxed">{v.desc}</div>
            </div>
          ))}
        </div>
            ...
            {contactStrip.map((item) => (
              <div key={item.label} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-[#1A202C] text-sm font-medium leading-relaxed">
                    {item.value}
                  </div>
                </div>
              </div>
